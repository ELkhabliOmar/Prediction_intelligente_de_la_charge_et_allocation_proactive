# train_lstm.py - ENTRAINEMENT LSTM (COMPATIBLE TEST.PY + CHECKPOINT ENRICHI)
import os
import csv
import argparse
import random
...
def build_pressure_series(workload: List[dict], fog_cpu: int) -> List[float]:
    ...
    # --- final clip realistic for training (do not let extreme dominate) ---
...
def make_windows(series: List[float], seq_len: int, stride: int = 1, augment: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
...
def create_dataloaders(X: torch.Tensor, y: torch.Tensor, batch_size: int = 64, val_ratio: float = 0.15, test_ratio: float = 0.15):
    n = X.shape[0]
...
def main():
    ...
    if not os.path.exists(args.data):
        raise FileNotFoundError(f"{args.data} introuvable. Lance d'abord generate_workload.py")
    ...
    if len(pressure_series) < args.seq_len + 30:
        raise RuntimeError(f"Série trop courte ({len(pressure_series)} points). Augmente la durée dans generate_workload.py")
    ...
    # IMPORTANT: normalisation identique à l'inference (clip norm à 2.0)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0 or patience == 0:
            ...
            print(f"[Epoch {epoch+1:3d}/{args.epochs}] Train={tr:.6f} | Val={va:.6f} {star} | LR={lr_now:.2e} | Best={best_val:.6f} (ep {best_epoch})")
    ...
    # --- checkpoint enrichi (compat test.py) ---
    try:
        import pandas as pd
